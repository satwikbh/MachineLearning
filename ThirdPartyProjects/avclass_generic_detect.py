...
def main(args):
    # Set input switch
    i_type = '-vt' if args.vt else '-lb'
    i_file = args.vt if args.vt else args.lb

    # Run avclass_labeler
    sys.stderr.write('[-] Running avclass_labeler on %s\n' % i_file)
    f_null = open(os.devnull, 'w')
    labeler = subprocess.Popen("python avclass_labeler.py %s %s -alias /dev/null -gen /dev/null -gendetect -gt %s" %
                               (i_type, i_file, args.gt), shell=True, stdout=f_null)
    labeler.wait()

    # Process generic tokens file
    sys.stderr.write('[-] Processing results.\n')
    gen_fname = os.path.basename(os.path.splitext(i_file)[0]) + '.gen'

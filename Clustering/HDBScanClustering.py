...
class HDBScanClustering:
    ...
    def prepare_hdbscan(self, dr_matrices, list_of_keys, variant_labels):
        ...
        for dr_name, dr_matrix in dr_matrices.items():
            if dr_name is "pca":
                ...
                self.log.info("Performing HDBSCAN on TSNE with random init")
            elif dr_name is "tsne_pca":
                self.log.info("Performing HDBSCAN on TSNE with pca init")
            elif dr_name is "sae":
                self.log.info("Performing HDBSCAN on SAE")
            else:
    ...
    def save_results(self, num_rows, dr_results_array, pca_results_path, mds_results_path, tsne_results_path,
        ...
        for dr_name, dr_results in dr_results_array.items():
            if dr_name is "pca":
                pca_fname = pca_results_path + "hdbscan_pca_" + str(num_rows)
                np.savez_compressed(pca_fname, dr_results)
            elif dr_name is "mds":
                mds_fname = mds_results_path + "hdbscan_mds_" + str(num_rows)
                np.savez_compressed(mds_fname, dr_results)
            elif dr_name is "nmds":
                nmds_fname = mds_results_path + "hdbscan_nmds_" + str(num_rows)
                np.savez_compressed(nmds_fname, dr_results)
            elif dr_name is "tsne_random":
                tsne_random_fname = tsne_results_path + "hdbscan_tsne_random" + str(num_rows)
                np.savez_compressed(tsne_random_fname, dr_results)
            elif dr_name is "tsne_pca":
                tsne_pca_fname = tsne_results_path + "hdbscan_tsne_pca" + str(num_rows)
                np.savez_compressed(tsne_pca_fname, dr_results)
            elif dr_name is "sae":
                sae_fname = sae_results_path + "hdbscan_sae_" + str(num_rows)

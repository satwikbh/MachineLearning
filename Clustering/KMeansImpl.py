    ...
    def prepare_kmeans(self, dr_matrices, list_of_keys, variant_labels):
        ...
        for dr_name, dr_matrix in dr_matrices.items():
            if dr_name is "pca":
                ...
                self.log.info("Performing K-Means on SAE")
            else:
    ...
    def save_results(self, num_rows, dr_results_array, pca_results_path, mds_results_path, tsne_results_path,
        ...
        for dr_name, dr_results in dr_results_array.items():
            if dr_name is "pca":
                ...
                tsne_random_fname = tsne_results_path + "kmeans_tsne_random_" + str(num_rows)
                np.savez_compressed(tsne_random_fname, dr_results)
            elif dr_name is "tsne_pca":
                tsne_pca_fname = tsne_results_path + "kmeans_tsne_pca_" + str(num_rows)
                np.savez_compressed(tsne_pca_fname, dr_results)
            elif dr_name is "sae":
                sae_fname = sae_results_path + "kmeans_sae_" + str(num_rows)

...
class StochasticGradientDescent:
    ...
    def compute_metrics(self, y_test, y_pred, dr_name, sgd_results_path):
        """
        ...
        if self.use_pruned_data:
            plt_path = sgd_results_path + "/" + str(dr_name) + "_" + str(
    ...
    def classification(self, train_data, train_labels):
        cv = 5
        self.log.info("Using SGD classifier")
        clf = SGDClassifier(random_state=0, n_jobs=30, verbose=1)
    ...
    def prepare_classifier(self, dr_matrices, labels, sgd_results_path):
        """
        ...
        for dr_name, dr_matrix in dr_matrices.items():
            if dr_name is "base_data":
                self.log.info("Using SGD classifier on Base Data")
            elif dr_name is "pca":
                self.log.info("Using SGD classifier on PCA")
            elif dr_name is "tsne_random":
                self.log.info("Using SGD classifier on TSNE with random init")
            elif dr_name is "tsne_pca":
                self.log.info("Using SGD classifier on TSNE with pca init")
            elif dr_name is "sae":
                self.log.info("Using SGD classifier on on SAE")
            else:
    ...
    def main(self, num_rows):
        ...
        bagging_results_path = self.config["models"]["sgd"]["results_path"]
        ...
        self.prepare_classifier(dr_matrices=dr_matrices, labels=labels, sgd_results_path=bagging_results_path)
        self.log.info("Total time taken : {}".format(time() - start_time))


if __name__ == '__main__':
    sgd_clf = StochasticGradientDescent(use_pruned_data=True)
    sgd_clf.main(num_rows=50000)

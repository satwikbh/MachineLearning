...
class ExtraTrees:
    def __init__(self, use_pruned_data):
        ...
        self.classifier_name = "extra_trees"

    def compute_metrics(self, y_test, y_pred, dr_name, et_results_path):
        """

        :param y_test:
        :param y_pred:
        :param dr_name:
        :param et_results_path:
        :return:
        """
        results = dict()
        ...
        if self.use_pruned_data:
            plt_path = et_results_path + "/" + str(dr_name) + "_" + str(
    ...
    def classification(self, train_data, train_labels):
        ...
        clf = ExtraTreesClassifier(n_estimators=10, max_depth=None, min_samples_split=2, random_state=0, n_jobs=3)
    ...
    def prepare_classifier(self, dr_matrices, labels, et_results_path):
        """
        ...
        for dr_name, dr_matrix in dr_matrices.items():
            if dr_name is "base_data":
                self.log.info("Using ExtraTrees classifier on Base Data")
            elif dr_name is "pca":
                self.log.info("Using ExtraTrees classifier on PCA")
            elif dr_name is "tsne_random":
                self.log.info("Using ExtraTrees classifier on TSNE with random init")
            elif dr_name is "tsne_pca":
                self.log.info("Using ExtraTrees classifier on TSNE with pca init")
            elif dr_name is "sae":
    ...
    def main(self, num_rows):
        ...
        et_results_path = self.config["models"]["extra_trees"]["results_path"]
        ...
        self.prepare_classifier(dr_matrices=dr_matrices, labels=labels, et_results_path=et_results_path)
        self.log.info("Total time taken : {}".format(time() - start_time))


if __name__ == '__main__':
    et = ExtraTrees(use_pruned_data=True)
    et.main(num_rows=50000)
